...
use Evrinoma\PartnerBundle\Factory\Partner\Factory as PartnerFactory;
use Evrinoma\PartnerBundle\Mediator\QueryMediatorInterface;
use Evrinoma\PartnerBundle\Repository\Partner\PartnerCommandRepositoryInterface;
...
class EvrinomaPartnerExtension extends Extension
{
    use HelperTrait;

    public const ENTITY = 'Evrinoma\PartnerBundle\Entity';
    public const MODEL = 'Evrinoma\PartnerBundle\Model';
    public const ENTITY_FACTORY_PARTNER = PartnerFactory::class;
    public const ENTITY_BASE_PARTNER = BasePartner::class;
    public const DTO_BASE_PARTNER = PartnerApiDto::class;
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        if (self::ENTITY_FACTORY_PARTNER !== $config['factory']) {
    ...
    private function wireConstraintTag(ContainerBuilder $container): void
    {
        foreach ($container->getDefinitions() as $key => $definition) {
            switch (true) {
                case str_contains($key, PartnerPass::PARTNER_CONSTRAINT)   :
                    $definition->addTag(PartnerPass::PARTNER_CONSTRAINT);
                    break;
                default:
            }
        }
    ...
    private function wireRepository(ContainerBuilder $container, Reference $registry, string $madiator, string $class, string $driver): void
    {
        ...
        $container->addAliases([PartnerCommandRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
        $container->addAliases([PartnerQueryRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
    }
    ...
    {
        return EvrinomaPartnerBundle::BUNDLE;
    }
}
